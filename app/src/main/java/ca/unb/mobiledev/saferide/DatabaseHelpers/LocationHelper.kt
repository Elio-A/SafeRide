...
class LocationHelper(context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION)  {
    ...
    fun addLocation(user_id: Int, station_id: Int, location: String): Long{
    ...
    fun getNextTrip(station_id: Int, seat: Int): List<Locations> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val todayDate = dateFormat.format(Date())

        val db = this.readableDatabase
        val columns = arrayOf(COLUMN_USER_ID, COLUMN_LOCATION)
        val selection = "station_id = ? and status = ? and DATE($COLUMN_TIME) = ?"
        val args = arrayOf(station_id.toString(), "0", todayDate)
